...
public class ContinentContainer {
    ...
    public List<Continent> findByContinentName(String p_continentName) {
        return UserCoreLogic.getGameEngine().getMapEditorEngine().getContinentList().stream()
                .filter(p_continent -> p_continent.getContinentName().equals(p_continentName))
                .collect(Collectors.toList());
    }

    /**
     * Performs a search for a continent by its name, returning the first match
     * found.
     *
     * @param p_continentName Value of the name of continent.
     * @return Value of the very first matched continents.
     * @throws EntityNotFoundException Throws If no continent with the specified
     *                                 name can be found.
     */
     ...
     * @throws EntityNotFoundException Throws If no continent with the given ID can
     *                                 be located.
     */
    public Continent findByContinentId(Integer p_continentId) throws EntityNotFoundException {
        List<Continent> l_continentList = UserCoreLogic.getGameEngine().getMapEditorEngine().getContinentList().stream()
                .filter(p_continent -> p_continent.getContinentId().equals(p_continentId)).collect(Collectors.toList());
